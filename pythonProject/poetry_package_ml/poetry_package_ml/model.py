...
def prepare_df(df):
   ...
   df.loc[df['Cabin_deck'].isna(), 'Cabin_deck'] = df[df['Group_size'] == 1].groupby(['HomePlanet', 'Destination', ])['Cabin_deck'].transform(lambda x: x.fillna(pd.Series.mode(x)[0]))[na_rows_CD]
   ...
   df.groupby(['HomePlanet', 'No_spending', 'Group_size', 'Cabin_deck', 'Cabin_side'])['Age'].mean().unstack().fillna(
      0)

   na_rows_A = df.loc[df['Age'].isna(), 'Age'].index
   df.loc[df['Age'].isna(), 'Age'] = df.groupby(['HomePlanet', 'No_spending', 'Group_size', 'Cabin_deck', 'Cabin_side'])['Age'].transform(lambda x: x.fillna(x.mean()))[na_rows_A]
   ...
   for col in ['RoomService', 'FoodCourt', 'ShoppingMall', 'Spa', 'VRDeck']:
      ...
      df.loc[df[col].isna(), col] = df.groupby(['HomePlanet', 'Group_size', 'Age_group'])[col].transform(lambda x: x.fillna(x.mean()))[na_rows]

   mode_fill = df.groupby(['HomePlanet', 'Group_size', 'Age_group', 'Destination', 'Expenditure'])['VIP'].transform(lambda x: x.mode().iat[0] if not x.mode().empty else False)
...
def predict():
   ...
   df_test_prepared = prepare_df(df_test)

   predict = model.predict(df_test_prepared)
